...
export const reactToMessage = (messageId: UUID, reaction: string, add: boolean, token: string) => async (dispatch: AppDispatch): Promise<void> => {
    try {
        const res: Response = await fetch(`${BASE_API_URL}/${MESSAGE_PATH}/${messageId}/react?reaction=${encodeURIComponent(reaction)}&add=${add}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `${AUTHORIZATION_PREFIX}${token}`,
