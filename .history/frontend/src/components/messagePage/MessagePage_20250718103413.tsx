...
interface MessagePageProps {
    ...
    onSendMessage: (replyToMessageId?: string | null) => void;
...
const MessagePage = (props: MessagePageProps) => {
    ...
    const getMessageCard = (message: MessageDTO) => {
        ...
        return <MessageCard message={message} reqUser={props.reqUser} key={message.id} isNewDate={isNewDate}
                            isGroup={props.chat.isGroup} onReply={setReplyToMessage}/>
    };
        ...
        <div className={styles.outerMessagePageContainer}>
            ...
            <div className={styles.messageContentContainer} onClick={onCloseEmojiPicker}>
                {messageQuery.length > 0 &&
                    sortedMessages.filter(x => x && x.id && x.content && x.user).filter(x => x.content.toLowerCase().includes(messageQuery))
                        .map(message => getMessageCard(message))}
                {messageQuery.length === 0 &&
                    sortedMessages.filter(x => x && x.id && x.content && x.user).map(message => getMessageCard(message))}
            ...
            <div className={styles.footerContainer}>
                {replyToMessage && (
                    <div className={styles.replyPreview}>
                        <span>
                            Replying to: <b>{replyToMessage.user?.fullName}</b>: {replyToMessage.content}
                        </span>
                        <IconButton onClick={() => setReplyToMessage(null)}>
                            <ClearIcon />
                        </IconButton>
                    </div>
                )}
                ...
                <div className={styles.innerFooterContainer}>
                    <TextField
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                                props.onSendMessage(replyToMessage?.id || null);
