...
interface MessageCardProps {
    ...
    usersInChat?: UserDTO[]; // Truyền từ cha nếu có
}
...
const MessageCard = (props: MessageCardProps) => {
    ...
    // Lấy danh sách user từ redux (chat hiện tại)
    const usersInChat: UserDTO[] = props.usersInChat || [];

    useEffect(() => {
      // Chỉ fetch tên cho các userId chưa có trong userNames
      const fetchNames = async () => {
        const missingIds = heartUsers.filter(
          (id) => !userNames[id]
        );
      ...
      if (openPopover && heartUsers.length > 0) fetchNames();
      // eslint-disable-next-line
    }, [openPopover, heartUsers]);
                    ...
                    <List>
                        {heartUsers.map((userId, idx) => (
                            <ListItem key={userId + idx}>
                                <ListItemText primary={userNames[userId] || userId} />
                            </ListItem>
                        ))}
                    </List>
                </Popover>
