...
const MessageCard = (props: MessageCardProps) => {
    ...
    // useEffect(() => {
    //   if (openPopover && heartUsers.length > 0) {
    //     const token = localStorage.getItem('token') || '';
    //     heartUsers.forEach((id) => {
    //       if (!authState.userById[id]) {
    //         reduxDispatch<any>(getUserById(id, token));
    //       }
    //     });
    //   }
                    ...
                    <List>
                        {heartUsers.map((userId, idx) => (
                            <ListItem key={userId + idx}>
                                <ListItemText primary={authState.userById[userId]?.fullName || userId} />
                            </ListItem>
                        ))}
                    </List>
                </Popover>
