...
const Homepage = () => {
    ...
    //State cho chat va message
    const [messages, setMessages] = useState<MessageDTO[]>([]);
    const [newMessage, setNewMessage] = useState<string>("");
    //State quản lí websocket va real-time messaging
    ...
    useEffect(() => {
        if (messageState.newMessage && stompClient && currentChat && isConnected) {
            ...
            //frontend gửi tin nhắn qua websocket đến endpoint /app/mesages
        }
    ...
    const onSendMessage = (replyToId?: string | null) => {
        if (currentChat?.id && token) {
            // Chỉ truyền replyTo nếu đúng định dạng UUID, hoặc undefined/null
            const replyTo: UUID | null | undefined = (replyToId && /^[0-9a-fA-F-]{36}$/.test(replyToId)) ? replyToId as UUID : null;
            dispatch(createMessage({chatId: currentChat.id, content: newMessage, replyTo}, token));
            setNewMessage("");
        }
    };
