...
public class MessageController {
    ...
    public ResponseEntity<MessageDTO> reactToMessage(@PathVariable UUID id,
                                                    @RequestParam String reaction,
                                                    @RequestParam boolean add,
                                                    @RequestHeader(JwtConstants.TOKEN_HEADER) String jwt) throws UserException, MessageException {
        User user = userService.findUserByProfile(jwt);
        Message message = messageService.reactToMessage(id, reaction, user.getId(), add);
        return new ResponseEntity<>(MessageDTO.fromMessage(message), HttpStatus.OK);
    }
}
