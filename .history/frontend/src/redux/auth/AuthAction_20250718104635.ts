...
export const getUserById = (id: string, token: string) => async (dispatch: AppDispatch): Promise<void> => {
    try {
        const res: Response = await fetch(`${BASE_API_URL}/api/users/${id}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `${AUTHORIZATION_PREFIX}${token}`,
        ...
        const resData: UserDTO = await res.json();
        dispatch({type: actionTypes.GET_USER_BY_ID, payload: resData});
    } catch (error: any) {
