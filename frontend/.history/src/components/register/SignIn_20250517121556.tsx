...
const SignIn = () => {
    ...
    //Dòng này lấy ra token JWT (nếu có) đã được lưu sau khi người dùng đăng nhập.
    //Token có thể là string hoặc null
    const state: AuthReducerState = useSelector((state: RootState) => state.auth);
    //lấy được trạng thái auth từ Redux store
    //state.auth là một slice trong Redux store, chứa thông tin về trạng thái đăng nhập của người dùng.
    useEffect(() => {
